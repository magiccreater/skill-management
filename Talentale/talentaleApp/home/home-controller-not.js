...
homeApp.controller('homeController', ['$scope', '$rootScope', '$cookieStore', '$http', '$q', '$sce', '$filter', 'talentaleAPI', 'contantValue', 'NgTableParams', 'userService', 'dialogService', 'messageService',
    function ($scope, $rootScope, $cookieStore, $http, $q, $sce, $filter, talentaleAPI, contantValue, NgTableParams, userService, dialogService, messageService) {
        ...
        $scope.setWeightonInit=function(userskill){
            userskill.weight == undefined ? userskill.weight = 5:userskill.weight;
            $scope.tempWeight=angular.copy(userskill.weight);
        };
        
        $scope.setWeightOnMouseLeave=function(tempWeight,userskill){
            userskill.weight =tempWeight;
        };
        ...
        var _saveSkills = function () {
            if (!$rootScope.showLoginSignUp) {
                ...
                var dialog = dialogService.openDialog("custom", { 'keyboard': true, 'backdrop': true, 'size': 'md' }, "auth/login/partials/login.html", "loginController", { data: $scope.jobSearchDetail });
